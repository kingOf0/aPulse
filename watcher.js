...
const sendSMTPMessage = async (text) => {
    try {
        ...
        return await transporter.sendMail({
            ...
            subject: config.smtp.subject, // Subject line
            text: config.smtp.text.replaceAll(/\${text}/g, text), // plain text body
            html: config.smtp.html.replaceAll(/\${text}/g, text), // html body
        });
    } catch (e) {
        console.error("Error while sending smtp mail: ", e)
    }
};
...
while (true) {
    ...
    try {
        ...
        for (let site of config.sites) {
            ...
            let i = 1;
            let siteId_ = siteId;
            while (siteIds.includes(siteId)) {
                siteId = siteId_ + '-' + (++i)
            } // Ensure a unique site id
            siteIds.push(siteId);
            ...
            try {
                for (let endpoint of site.endpoints) {
                    ...
                    let i = 1;
                    let endpointId_ = endpointId;
                    while (endpointIds.includes(endpointId)) {
                        endpointId = endpointId_ + '-' + (++i)
                    } // Ensure a unique endpoint id
